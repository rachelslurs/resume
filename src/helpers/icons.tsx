...
const DynamicIcon: React.ComponentType<Props & IconChoice> = ({
  iconSelected,
  className,
  transform,
  ...props
        ...
        import(`@fortawesome/free-solid-svg-icons`).then(
          (lib) => {
            if (typeof iconSelected === 'string' && iconSelected in lib) {
              const IconComponent: React.ComponentType<Props> = (props) => (
                <FontAwesomeIcon
                  icon={lib[iconSelected as keyof typeof lib] as IconDefinition}
                  {...props}
                />
              )
          ...
          (reason) => {
            ...
            const DefaultComponent: React.ComponentType<Props> = () => (
              <div>Loading</div>
            )
