...
import { getMatchingGrayColor } from './helpers'
import { ThemeOwnProps, themePropDefs } from './props/index.js';

import type { ComponentPropsWithout, RemovedProps } from './helpers/index.js';
import { getColor, getTheme } from 'src/app/actions.client.js';
...
const ThemeRoot = React.forwardRef<ThemeImplElement, ThemeImplPublicProps>(
  (props, forwardedRef) => {
    ...
    React.useEffect(() => setAppearance(theme as ThemeAppearance || appearanceProp), [appearanceProp, theme]);

    const [accentColor, setAccentColor] = React.useState(accentColorProp);
    React.useEffect(() => setAccentColor(color as ThemeAccentColor || accentColorProp), [accentColorProp]);
...
const ThemeImpl = React.forwardRef<ThemeImplElement, ThemeImplProps>((props, forwardedRef) => {
  ...
  const {
    ...
    //
    appearance = context?.appearance ?? (theme || themePropDefs.appearance.default),
